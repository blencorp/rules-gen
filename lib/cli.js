...
class CLI {
  ...
  async processRules(category, rules, ideType, force = false) {
    ...
    try {
      for (const rule of rules) {
        ...
        if (ideType === 'cursor') {
          result = await cursorGenerator.generate(rule, outputPath, force);
        } else if (ideType === 'windsurf') {
          result = await windsurfGenerator.generate(rule, outputPath, force);
        }
